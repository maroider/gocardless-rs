...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let linked_requisition = requisitions
        .results
        .iter()
        .find(|requisition| requisition.status == "LN")
        .unwrap();
    dbg!(&linked_requisition);

    dbg!(linked_requisition.accounts.len());

    for account_id in &linked_requisition.accounts {
        dbg!(&account_id);
